    ...
    <div className='bg-[#faf9fb] min-h-screen flex flex-col'>
      ...
      {showToast && (
        <div className='fixed top-6 left-1/2 transform -translate-x-1/2 z-50 bg-[#23233a] text-white px-6 py-3 rounded-lg shadow-lg flex items-center gap-2 animate-fadeInToast'>
          <svg className='w-5 h-5 text-[#D4AF37]' fill='none' stroke='currentColor' strokeWidth='2' viewBox='0 0 24 24'><path strokeLinecap='round' strokeLinejoin='round' d='M5 13l4 4L19 7' /></svg>
      ...
      <main className='flex-grow max-w-5xl mx-auto w-full px-4 py-12 flex flex-col md:flex-row gap-12'>
        {/* Left: Product Image */}
        <div className='flex-1 flex items-center justify-center'>
          <img src={product.image} alt={product.title} className='w-full max-w-xs rounded-xl shadow-lg object-cover'/>
        </div>
        {/* Right: Product Info */}
        <div className='flex-1 flex flex-col justify-center'>
          <h1 className='text-2xl md:text-3xl font-serif font-bold mb-2'>{product.title}</h1>
          <div className='text-[#D4AF37] text-xl font-serif font-semibold mb-2'>{product.price}</div>
          <div className='flex items-center mb-2'>
            {/* Stars */}
            {[...Array(5)].map((_, i) => (
              <svg key={i} width='20' height='20' fill={i < 4 ? '#D4AF37' : 'none'} stroke='#D4AF37' strokeWidth='1.5' viewBox='0 0 24 24'>
          ...
          <div className='mb-6'>
            ...
            <ul className='bg-white border border-gray-200 rounded-lg p-4 text-sm space-y-2'>
          ...
          <div className='flex gap-4'>
            <button
              className={`px-8 py-3 font-serif rounded transition ${isInCart ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-[#D4AF37] text-white hover:brightness-110'}`}
              onClick={async () => {
                if (!isInCart) {
            ...
            <button
              className='px-8 py-3 font-serif rounded transition bg-[#18182a] text-white hover:brightness-110'
              onClick={async () => {
                if (!isInCart) {
                  await addToCart(product)
                }
      ...
      <section className='max-w-5xl mx-auto w-full px-4'>
        <div className='border-b border-gray-200 flex gap-8 mb-6'>
          <button
            className={`py-3 px-4 font-semibold border-b-2 transition ${tab === 'specs' ? 'border-[#D4AF37] text-[#18182a]' : 'border-transparent text-gray-500'}`}
            onClick={() => setTab('specs')}
          >
            Specifications
          </button>
          <button
            className={`py-3 px-4 font-semibold border-b-2 transition ${tab === 'reviews' ? 'border-[#D4AF37] text-[#18182a]' : 'border-transparent text-gray-500'}`}
        ...
        {tab === 'specs' && (
          <div className='mb-10 text-gray-700'>{product.longDescription || product.description}</div>
        )}
        {tab === 'reviews' && (
          <div className='mb-10'>
            {/* Review Form */}
            {!isAuthenticated ? (
              <div className='mb-8 p-4 bg-white rounded shadow text-center'>
                <span className='text-gray-700'>You must be <button className='text-[#D4AF37] underline' onClick={() => window.location.href = '/login'}>logged in</button> to submit a review.</span>
              </div>
            ) : (
              <form
              ...
              >
                ...
                <button
                  type='submit'
                  className='self-end px-6 py-2 bg-[#D4AF37] text-white rounded font-semibold hover:brightness-110 transition disabled:opacity-60'
                  disabled={submitting}
                >
            ...
            {!reviewsLoading && !reviewsError && reviews.map((r, i) => (
              <div key={i} className='mb-4 p-4 bg-white rounded shadow'>
                ...
                <div className='flex items-center mb-1'>
                  {[...Array(5)].map((_, j) => (
                    <svg key={j} width='16' height='16' fill={j < r.rating ? '#D4AF37' : 'none'} stroke='#D4AF37' strokeWidth='1.2' viewBox='0 0 24 24'>
                      <polygon points='12,2 15,9 22,9.5 17,14.5 18.5,22 12,18 5.5,22 7,14.5 2,9.5 9,9' />
                    </svg>
                  ))}
                </div>
