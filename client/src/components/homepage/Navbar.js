  ...
  return (
    <nav className='sticky top-0 w-full h-16 flex items-center justify-between px-4 bg-white shadow z-50'>
      ...
      <div className='flex items-center gap-2 min-w-0 cursor-pointer' onClick={() => navigate('/')}> 
        ...
        <span className='text-black font-serif text-sm sm:text-base font-semibold tracking-wide whitespace-nowrap'>KAIVALYA ASTROLOGY</span>
      </div>
      {/* Desktop Navigation */}
      <div className='hidden md:flex items-center w-full'>
        ...
        <div className='flex-1 flex justify-center gap-6'>
          {navLinks.map(link => (
            <button
              key={link.label}
              className={`text-black font-serif text-base font-semibold tracking-wider transition px-2 py-1
                ${location.pathname.startsWith(link.path) ? 'underline underline-offset-4' : ''}
                hover:underline hover:underline-offset-4 hover:text-[#003D37]`}
        ...
        <div className='flex items-center gap-3'>
          {isAuthenticated && (
            <button className='relative flex items-center gap-1 px-2 py-1' onClick={() => handleNavigate('/cart')} aria-label='Cart'>
              ...
              {cartItems.length > 0 && (
                <span className='ml-1 bg-black text-white text-xs rounded-full px-2 py-0.5 font-bold'>{cartItems.length}</span>
              )}
            </button>
          )}
          {isAuthenticated ? (
            <>
              <button className='px-3 py-1 bg-black text-white font-serif rounded hover:bg-[#003D37] transition text-base' onClick={logout}>Logout</button>
              <button className='px-3 py-1 bg-black text-white font-serif rounded hover:bg-[#003D37] transition text-base' onClick={() => handleNavigate('/profile')}>Profile</button>
            </>
          ) : (
            <>
              <button
                className='px-3 py-1 bg-transparent border border-black text-black font-serif rounded hover:bg-[#003D37] hover:text-white transition text-base'
                onClick={() => handleNavigate('/login')}
              >
                Sign In
              </button>
              <button
                className='px-3 py-1 bg-black text-white font-serif rounded hover:bg-[#003D37] transition text-base'
      ...
      <button
        className='flex items-center justify-center p-2 rounded-md text-black hover:bg-gray-100 transition ml-auto md:hidden'
      ...
      <div className={`fixed top-0 right-0 h-full w-64 bg-white shadow-lg z-50 transform transition-transform duration-300 ease-in-out ${menuOpen ? 'translate-x-0' : 'translate-x-full'} md:hidden`}>
